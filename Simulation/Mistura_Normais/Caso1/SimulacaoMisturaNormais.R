...
######                          Mistura de Normais                                         ######
######      N(0,0;1,1;0.9)           /       N(0,0;1,1;0.5)                                ######
#################################################################################################

############### Caso 1 0.9 x N_1 + 0.1 x N_2
...
##### Inicio do Monte Carlo ######
  ...
  for (j in 1:4){
    ...
    if (j==4) dados4=cbind(ENREP1,ENREP2,ENREP3,ENREP4,ENREP5) ## Matrix com os varios n para gama=0.1
    
  }

setwd("/home/cleber/Dropbox/Dissertacao-Cleber/Simulacao/R/Simulacao 17⁄11⁄2013/MisturaNormais/Caso1/")

  #############  Impressao dos resultados  #################
  
    ## Saindo tabela
    write.table(round(rbind(Rho_Gama,media_rho_mc,media_rho_ch,media_rho_sp),4),file="EstimativaMisturaCaso1.csv"
                ,row.names=FALSE,sep=";",dec=".",col.names=c("10","50","100","250","500"))
    write.table(round(rbind(vies_rho_mc,vies_rho_ch,vies_rho_sp),4),file="ViésMisturaCaso1.csv"
                ,row.names=FALSE,sep=";",dec=".",col.names=c("10","50","100","250","500"))
    write.table(round(rbind(var_rho_mc,var_rho_ch,var_rho_sp),4),file="VarianciaMisturaCaso1.csv"
                ,row.names=FALSE,sep=";",dec=".",col.names=c("10","50","100","250","500"))
    write.table(round(rbind(raiz_eqm_rho_mc,raiz_eqm_rho_ch,raiz_eqm_rho_sp),4),file="ReqmMisturaCaso1.csv"
                ,row.names=FALSE,sep=";",dec=".",col.names=c("10","50","100","250","500"))
...
setwd("/home/cleber/Dropbox/Dissertacao-Cleber/Simulacao/Graficos/colorido/Simulacao 17⁄11⁄2013/MisturaNormais/Caso1/")  
save.image("RhoMistura.Rdata")
load("~/Dropbox/Dissertacao-Cleber/Simulacao/Graficos/colorido/Simulacao 17⁄11⁄2013/MisturaNormais/Caso1/RhoMistura.Rdata")  
library(reshape2)
library(ggplot2)
